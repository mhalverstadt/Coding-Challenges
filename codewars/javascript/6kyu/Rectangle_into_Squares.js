...
function sqInRect(lng, wdth){
    ...
    let result = []
    let area = lng * wdth
    let shortSide = lng > wdth ? wdth : lng
    let longSide = lng === shortSide ? wdth : lng
    while(area !== 0){
        result.push(shortSide)
        area -= shortSide**2
        let prevLongSide = longSide
        let prevShortSide = shortSide
        longSide = longSide - shortSide > shortSide ? longSide - shortSide : shortSide
        shortSide = shortSide > prevLongSide - shortSide ? prevLongSide - shortSide : shortSide
    }
    return result
}
